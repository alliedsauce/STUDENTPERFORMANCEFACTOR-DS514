...
### **📊 Project Overview**
- **Project Objective:** วิเคราะห์ปัจจัยที่มีผลต่อคะแนนสอบ และสร้างโมเดลทำนายกลุ่มคะแนนสูง/ต่ำ
- **Dataset:** 6,607 records พร้อม 9 features
- **Tools & Libraries:** Python, scikit-learn, pandas, seaborn
- **Goal:** สร้างโมเดลที่แม่นยำและตีความง่าย เพื่อช่วยวางแผนการเรียนการสอน
 ...
 **🧮 Encoding**
 ...
 - **Encoding:** Ordinal สำหรับระดับ (Low/Medium/High), Binary สำหรับ Yes/No
 - **Scaling:** StandardScaler สำหรับฟีเจอร์เชิงตัวเลข
...
- แปลว่า: แบบจำลองระบุกลุ่มคะแนนต่ำ (P) ได้ถูกต้อง

**❌➕False Positive (FP)** = 81 (บนขวา)
- แปลว่า: แบบจำลองระบุกลุ่มคะแนนต่ำ (P) ผิดพลาด

**Label 0** (Negative Class) ⇒ กลุ่มคะแนนสูง

**✔️➖True Negative (TN)** = 244 (บนซ้าย)
- แปลว่า: แบบจำลองระบุกลุ่มคะแนนสูง (N) ได้ถูกต้อง

**❌➖False Negative (FN)** = 37 (ล่างซ้าย)
- แปลว่า: แบบจำลองระบุกลุ่มคะแนนสูง (N) ผิดพลาด
...
- คะแนนสูง(0): 0.81 → ปานกลาง (เพราะ Recall ต่ำกว่า Precision)
- คะแนนต่ำ(1): 0.94 → ดีมาก (ทั้ง Precision และ Recall สูง)


**🌟 Insights**
- โมเดลทำงานได้ดีมากกับ **กลุ่มคะแนนต่ำ** (Recall = 0.96, F1 = 0.94)
- **กลุ่มคะแนนสูง** มี Precision ดี (0.87) แต่ Recall ต่ำ (0.75) → โมเดลพลาดบางคนที่ควรอยู่ในกลุ่มคะแนนสูง
- Accuracy รวม = 91% ถือว่าดี แต่มี Class Imbalance (997 vs 325)

**🧙 Next Steps**
- ปรับ Threshold เพื่อเพิ่ม Recall ของกลุ่มคะแนนสูง
